...
final class BaiduDriveAdapterTests: XCTestCase {
    ...
    func testPollPendingThenAuthorized() async throws {
        ...
        let client = BaiduMockHTTPClient(responses: [(200, qr), (200, pending), (200, confirmed), (200, profile)])
        client.responseHeaders = ["Set-Cookie": "BDUSS=abc123"]
        let auth = BaiduAuthService(client: client)
        let session = BaiduSession(store: store, service: auth)
        let adapter = BaiduDriveServiceAdapter(session: session, auth: auth, threadStore: FishThreadStore(defaults: UserDefaults(suiteName: "test.baidu.poll")!))
        let scanSession = try await adapter.beginLogin()
        let first = try await adapter.poll(scanSession)
        XCTAssertEqual(first, .pending)
        let second = try await adapter.poll(scanSession)
